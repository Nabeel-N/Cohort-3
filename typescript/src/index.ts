...
const add = cart.reduce((acc: number, current) => {
...
const revenue = sales.reduce((acc: { [key: string]: number }, current) => {
...
const app: { facebook: string; instagram: number } = {
  facebook: "nabeel",
  instagram: 8497,
};
...
const developer: { [key: string]: number } = {
  framework: 4231,
  library: 24323,
};
